...
async function sendRejectionEmail(toEmail: string): Promise<void> {
  await resendEmail(toEmail, "Your Parliament AI access request",
    "<div style=\"font-family:sans-serif;max-width:480px;margin:auto\"><h2>Access request update</h2><p>Thank you for your interest in Parliament AI. After reviewing your request, we are unable to approve access at this time.</p><p>If you believe this is an error, please contact <a href=\"mailto:" + process.env.SUPPORT_EMAIL + "\">" + process.env.SUPPORT_EMAIL + "</a>.</p></div>"
  );
}
async function sendSuspensionEmail(toEmail: string, displayName: string): Promise<void> {
  ...
  await resendEmail(toEmail, "Your Parliament AI account has been suspended",
    "<div style=\"font-family:sans-serif;max-width:480px;margin:auto\"><h2>Account suspended</h2><p>Hi " + name + ",</p><p>Your Parliament AI account has been suspended. You will not be able to sign in until your account is reactivated.</p><p>If you believe this is an error, please contact <a href=\"mailto:" + process.env.SUPPORT_EMAIL + "\">" + process.env.SUPPORT_EMAIL + "</a>.</p></div>"
  );
}
