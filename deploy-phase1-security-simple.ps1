...
gcloud iam service-accounts create parliament-api-foundation --display-name="Parliament API Foundation Service Account" --description="Foundation service account for Parliament API with minimal permissions" --project=$PROJECT_ID

Write-Host "   Creating Frontend service account..."
gcloud iam service-accounts create parliament-frontend-foundation --display-name="Parliament Frontend Foundation Service Account" --description="Foundation service account for Parliament Frontend with minimal permissions" --project=$PROJECT_ID

# 2. Grant IAM Roles
Write-Host ""
Write-Host "2. Granting IAM Roles..." -ForegroundColor Yellow

Write-Host "   Granting Firestore access to API service account..."
gcloud projects add-iam-policy-binding $PROJECT_ID --member="serviceAccount:parliament-api-foundation@$PROJECT_ID.iam.gserviceaccount.com" --role="roles/datastore.user"

Write-Host "   Granting logging access to API service account..."
gcloud projects add-iam-policy-binding $PROJECT_ID --member="serviceAccount:parliament-api-foundation@$PROJECT_ID.iam.gserviceaccount.com" --role="roles/logging.logWriter"

Write-Host "   Granting monitoring access to API service account..."
gcloud projects add-iam-policy-binding $PROJECT_ID --member="serviceAccount:parliament-api-foundation@$PROJECT_ID.iam.gserviceaccount.com" --role="roles/monitoring.metricWriter"

# 3. Create Secret Manager Secret
Write-Host ""
Write-Host "3. Creating Secret Manager Secret..." -ForegroundColor Yellow
...
Write-Output $API_KEY | gcloud secrets create parliament-api-key --data-file=- --replication-policy="user-managed" --locations=$REGION --project=$PROJECT_ID

Write-Host "   Granting secret access to API service account..."
gcloud secrets add-iam-policy-binding parliament-api-key --member="serviceAccount:parliament-api-foundation@$PROJECT_ID.iam.gserviceaccount.com" --role="roles/secretmanager.secretAccessor" --project=$PROJECT_ID

# 4. Create Security Logs Bucket
Write-Host ""
Write-Host "4. Creating Security Logs Storage..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "5. Ensuring Required APIs are Enabled..." -ForegroundColor Yellow
